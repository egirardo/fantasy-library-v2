...
export default function BookListCard({ book, initialSaved }: Props) {
    const [errored, setErrored] = useState(false);
    const saveBookWithId = saveBook.bind(null, book.id);
    const [saved, action] = useActionState(saveBookWithId, initialSaved);

    return (
        <div className="relative w-full bg-surface border border-blue-900/50 rounded-lg shadow-md hover:shadow-xl hover:border-blue-500 hover:bg-surface-hover transition-all">
            ...
            <button
                ...
                className="absolute top-3 right-3 text-blue-400 hover:text-blue-300 transition-colors z-10"
                aria-label={saved ? "Unsave book" : "Save book"}
            >
                {saved ? <FaHeart size={22} /> : <FaRegHeart size={22} />}
            </button>
        </div>
    );
}
